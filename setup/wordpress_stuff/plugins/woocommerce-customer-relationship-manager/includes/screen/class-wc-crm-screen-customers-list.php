...
if (!class_exists('WC_CRM_Screen_Customers_List')) :
    ...
    class WC_CRM_Screen_Customers_List
    {
        ...
        public static function get_sql($args = array())
        {
            ...
            if (isset($_REQUEST['agent']) && !empty($_REQUEST['agent']) && $_REQUEST['agent'] != -1) {
                $inner[] = "INNER JOIN
                (SELECT cust.`c_id`
            ...
            if (in_array('agent', $current_user->roles)) {
                ...
                ON cm.`wc_crm_customer_id` = cust.`c_id` 
                AND cm.`meta_key` = 'customer_agent'
                AND cm.`meta_value` = '{$current_user->ID}'
                ) c
                ON c.`c_id` = customer.`c_id`
                ";
            }
	      ...
	      inner join  {$wpdb->prefix}woocommerce_order_itemmeta as product on ( product.order_item_id = order_items.order_item_id and product.meta_key = '_product_id' ) ";
            ...
            $sql = "SELECT 
                ...
                LEFT JOIN {$wpdb->usermeta} usm
                ON customer.user_id = usm.user_id
                AND usm.meta_key = 'customer_agent'
                LEFT JOIN {$wpdb->usermeta} usm2
                ON usm.meta_value = usm2.user_id
                AND usm2.meta_key = 'first_name'
                LEFT JOIN {$wpdb->usermeta} usm3
                ON usm.meta_value = usm3.user_id
                AND usm3.meta_key = 'last_name'
                LEFT JOIN {$wpdb->usermeta} usm4
                ON customer.user_id = usm4.user_id
                AND usm4.meta_key = 'date_of_birth'
                LEFT JOIN {$wpdb->users} a_us
                ON usm.meta_value = a_us.ID
                LEFT JOIN {$wpdb->usermeta} usm5
                ON customer.user_id = usm5.user_id
                AND usm5.meta_key = 'billing_phone'
	    		{$join}
	    		{$inner}
    			WHERE 1=1
    			{$user_role_filter}
