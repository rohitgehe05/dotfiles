...
class WC_CRM_Screen_Call
{
    ...
    public static function get_info_fields($id)
    {
        ...
        if ($customer) {
            //todo fix this
            $the_customer = wc_crm_get_customer($customer, 'user_id');
            $the_customer = new WC_CRM_Customer($the_customer->c_id);
            $name = $the_customer->get_name();
            $user_string = esc_html($name) . ' (#' . absint($customer) . ' &ndash; ' . sanitize_email($the_customer->customer->email) . ')';
        } else {
            $customer = '';
        }
        ...
        $info_fields = apply_filters('wcrm_calls_info_fields', array(
            ...
            'dropdown_customers' => array(
                ...
                'type' => 'select',
                'options' => array(
                    $customer => htmlspecialchars($user_string)
                ),
                'custom_attributes' => array(
            ...
            'product' => array(
                ...
                'type' => 'select',
                'options' => array(
                    $product_id => htmlspecialchars($product_string)
                ),
                'custom_attributes' => array(
                    'data-allow_clear' => 'true',
                    'autocomplete' => 'off',
            ...
            'order' => array(
                ...
                'type' => 'select',
                'options' => array(
                    $order_id => htmlspecialchars($order_string)
                ),
                'custom_attributes' => array(
                    'data-allow_clear' => 'true',
                    'autocomplete' => 'off',
            ...
            'account' => array(
                ...
                'type' => 'select',
                'options' => array(
                    $account['id'] => htmlspecialchars($account['title'])
                ),
                'custom_attributes' => array(
                    'data-allow_clear' => 'true',
                    'autocomplete' => 'off',
                    'data-action' => 'wc_crm_json_search_accounts',
                    'data-placeholder' => __('Search for an account&hellip;', 'wc_crm'),
                ),
            ),
            'post_content' => array(
