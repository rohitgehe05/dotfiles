...
class WC_CRM_Table_Customers extends WP_List_Table
{
    ...
    function get_columns()
    {
        ...
        $columns = array(
            ...
            'location' => __('Location', 'wc_crm'),
            'date_of_birth' => __('DOB', 'wc_crm'),
    ...
    function column_customer_agent($item)
    {
        ...
        if ($item['customer_agent']) {
            ...
            $content = "<a href='admin.php?page=" . WC_CRM_TOKEN . "&c_id=" . $agent->c_id . "'>" . $agent->first_name . ' ' . $agent->last_name . "</a><br><small class='meta'>{$item['agent_username']}</span>";
        }
        return ($content);
    }

    function column_date_of_birth($item)
    {
