...
class WC_CRM_Screen_Task
{
    ...
    public static function get_info_fields($id)
    {
        ...
        if (!$edit && isset($_GET['c_id']) & !empty($_GET['c_id'])) {
            $customer_user = wc_crm_get_customer((int)$_GET['c_id']);
            $customer = $customer_user->user_id;
            $the_customer = new WC_CRM_Customer($customer_user->c_id);
        ...
        if ($customer) {
            $customer_user = wc_crm_get_customer($customer, 'user_id');
            $the_customer = new WC_CRM_Customer($customer_user->c_id);
            $name = $the_customer->get_name();
            $user_string = esc_html($name) . ' (#' . absint($customer_user->user_id) . ' &ndash; ' . sanitize_email($the_customer->email) . ')';
        } else {
            $customer = '';
        }
