...
jQuery(document).ready(function($){
  function plotAccordingToChoices() {
    ...
    */
    ...
    $('#graph-selector input:checked').each(function(ind, el) {
      selected_graphs.push($(el).val());
    });
    $.each(datasets, function(key, val) {
      if ($.inArray(datasets[key].optin_type, selected_graphs) != -1) {
  ...
  $('#graph-selector input').on('change', function() {
